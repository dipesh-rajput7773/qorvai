    ...
    <section className="py-32" id="showcase">
      <div className="max-w-[1200px] mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-20 gap-8">
          ...
          >
            We don&apos;t just build scripts. We engineer revenue-generating
            ecosystems that integrate deeply with your current stack.
          </motion.p>
        </div>
              ...
              <Link href={`/blog/${p.blogSlug}`} className="block cursor-pointer">
                ...
                >
                  <div className="absolute inset-x-8 bottom-8 flex justify-between items-end">
                    <div className="flex gap-2">
                      {p.tags.map((tag) => (
                        <span
                          key={tag}
                          className="px-3 py-1 bg-black/40 backdrop-blur-md border border-white/5 rounded-full text-[0.6rem] font-bold tracking-widest text-[#F2EDE8]"
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  <div className="absolute top-8 right-8 bg-[#C8714A]/10 border border-[#C8714A]/20 px-4 py-2 rounded-full flex items-center gap-2">
                    ...
                    <span className="text-[0.65rem] font-extrabold text-[#E8A882] uppercase tracking-tighter">
                      {p.metric}
                    </span>
                  </div>
                </div>
                <p className="text-[#8A857E] font-bold text-xs uppercase tracking-widest mb-2">
                  {p.client}
                </p>
